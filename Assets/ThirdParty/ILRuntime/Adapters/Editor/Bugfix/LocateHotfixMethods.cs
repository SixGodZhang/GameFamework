...
namespace GameFramework.Taurus
{
    ...
    public class LocateHotfixMethods:Editor
    {
        ...
        {
            string mainPath = EditorPrefs.GetString("main_assembly_path");
        ...
        private static void GeneratorDeledateStatements(Dictionary<DelegateInfo, List<ProtectMethodInfo>> delegateAndMethods)
        {
            ...
            foreach (var key in delegateAndMethods.Keys)
            {
                ...
                {
                    mis.Append("public static BugfixHelper." + key.DelegateName + " " + mi.MethodName + "__Delegate;\n\t\t\t");
                }
            }
            ...
            //模板路径
            var tmpdPath = Application.dataPath + "/ThirdParty/ILRuntime/Adapters/Editor/Adapter/Template/bugfix_delegate_statements.tmpd";
        ...
        private static void GeneratorDeledateDefines(List<DelegateInfo> delegateInfos)
        {
            ...
            //模板路径
            var tmpdPath = Application.dataPath + "/ThirdParty/ILRuntime/Adapters/Editor/Adapter/Template/bugfix_delegate_defines.tmpd";
            BugfixFileManager manager = new BugfixFileManager(tmpdPath);
            manager.SetKeyValue("{$Namespace}", "ILRuntime");
            ...
            foreach (var type in targetTypes)
            {
                ...
                foreach (var method in methods)
                {
                    ...
                    foreach (var param in parameters)
                    {
                        ...
                        parameter = (string.IsNullOrWhiteSpace(paramPrefix) ?"": paramPrefix + "_") + paramData.ParamTypeStr + "_";
                    ...
                    //方法名称
                    methodInfo.MethodName = (method.DeclaringType.ToString() + "_" + method.Name + "_IN_" + parameter.TrimEnd('_') + "_OUT_" + methodInfo.ReturnTypeStr).Replace(".", "_").TrimEnd('_');
